...
pub async fn start_process(
    ...
    if cfg!(target_os = "windows") {
        _start_process_windows(executable, arguments, working_directory, stream_sink).await;
    } else {
        _start_process_unix_like(executable, arguments, working_directory, stream_sink).await;
    }
}

#[cfg(target_os = "windows")]
async fn _start_process_windows(
    executable: String,
    arguments: Vec<String>,
    working_directory: String,
    stream_sink: StreamSink<String>) {
...
async fn _start_process_unix_like(
    executable: String,
    arguments: Vec<String>,
    working_directory: String,
    stream_sink: StreamSink<String>) {
