...
import 'package:xfdm/common/conf/binary_conf.dart';
import 'package:xfdm/common/utils/provider.dart';
import 'package:xfdm/common/rust/api/process_api.dart' as rs_process;
...
class Aria2cModel extends _$Aria2cModel {
  ...
  Aria2cModelState build() {
    ...
    state = Aria2cModelState(aria2cDir: aria2cDir);
    initAria2c();
    return state;
  }

  Future initAria2c() async {
    try {
      dPrint("launch Aria2c daemon");
      await _launchDaemon(appGlobalState.applicationBinaryModuleDir!);
    } catch (e) {
      dPrint("Aria2cManager.checkLazyLoad Error:$e");
    }
  }

  Future _launchDaemon(String applicationBinaryModuleDir) async {
    if (state.aria2c != null) return;
    await BinaryModuleConf.extractModule(
        ["aria2c"], applicationBinaryModuleDir);

    /// kill for debug hot reload
    if (kDebugMode) {
      Process.runSync("taskkill", ["/f", "/im", "aria2c.exe"]);
    }
  ...
  Future<void> _onLaunch(int port, String pwd) async {
